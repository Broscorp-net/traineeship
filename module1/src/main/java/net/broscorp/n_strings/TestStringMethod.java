...
public class TestStringMethod {
    ...
    public void testsplitWordsAndLetters(){
        String expectedString = "Слово Hello имеет - 5 букв. Слово World имеет - 5 букв. Количество слов - 2.";
        assertEquals(expectedString, numberOfWordsAndNumberOfLettersInEachWord(mainWords));
    }
    ...
    public void testingSong(){
        ...
        assertEquals(numberOfCouplet * 24, splitWords(songAboutBugs(numberOfBags, numberOfCouplet)).length);
    }
    ...
    private String numberOfWordsAndNumberOfLettersInEachWord(String string){
        StringBuilder result = new StringBuilder();
        for (String str : splitWords(string)){
            result.append("Слово ").append(str).append(" имеет - ").append(str.length()).append(" букв. ");
        }
        return result.append("Количество слов - ").append(splitWords(string).length).append(".").toString();
    }
    ...
    private String songAboutBugs(int numberOfBags, int numberOfCouplet){
        ...
        for(int i = 0; i < numberOfCouplet; i++){
            int m = 0;
            if(numberOfBags == 0 || numberOfBags < 0)
                m =0;
            m = numberOfBags - 10 + random.nextInt(20);
